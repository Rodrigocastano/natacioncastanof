...
import { PagoEntrenadoresService, PagoUsuario } from '../service/pago-entrenadores.service';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';

@Component({
  selector: 'app-reporteentrenador',
  imports: [
    CommonModule,
    FormsModule,
    CalendarModule,
  ...
  templateUrl: './reporteentrenador.component.html',
})
export class ReporteentrenadorComponent implements OnInit {
  ...
  constructor(private comproService: PagoEntrenadoresService) {}
  ...
  buscarPagos(): void {
    ...
    this.comproService.getPagosPorEntrenador(inicio, fin, this.usuarioSel.id).subscribe({
      next: data => {
        this.pagos = data;
  ...
  descargarPDF(): void {
    ...
    img.onload = () => {
      ...
      doc.setFontSize(16).text('Comprobante de pagos del entrenador', x + logoW + 10, y + 5);
      doc.setFontSize(11).setTextColor(100)
         .text('Club Natación Castaño', x + logoW + 10, y + 12)
         .text(
           `Entrenador: ${this.usuarioSel.nombreCompleto}`,
           x + logoW + 10, y + 19
         );
    ...
    autoTable(doc, {
      ...
      doc.save(`comprobante de entrenador${this.usuarioSel.id}.pdf`);
    };
  }
