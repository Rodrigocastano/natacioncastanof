...
export class AsistenciaComponent implements OnInit {
  ...
  // Propiedades para el manejo de grupos y turnos
  grupos: Grupo[] = [];
...
loadUsuariosForAsistencia() {
  ...
  this.asistenciaService.getAllTodoAsistencias().subscribe({
    next: (data) => {
      ...
      // Agrupar por grupo y turno
      const gruposMap = new Map<string, Asistencia[]>();

      this.asistencia.forEach((asi) => {
        // Asumo que asi tiene 'grupo' y 'turno' como propiedades, si no las tienes, debes agregarlas al modelo o hacer join en backend
